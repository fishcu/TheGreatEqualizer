...
object ImagePipeline {
    ...
    // ---------------------------------------------------------------
    // Precomputed sRGB transfer LUTs
    // ---------------------------------------------------------------

    /** EOTF LUT: 256 entries mapping uint8 sRGB [0..255] → linear float [0,1]. */
    private val EOTF_LUT: FloatArray by lazy {
        FloatArray(256) { i ->
            val v = i / 255f
            if (v <= 0.04045f) {
                v / 12.92f
    ...
    /**
     * OETF LUT: 4096 entries mapping linear float index → sRGB uint8 [0,255].
     * Index with `(linearValue * 4095f).toInt().coerceIn(0, 4095)`.
     */
    private val OETF_LUT: IntArray by lazy {
        IntArray(4096) { i ->
            val v = i / 4095f
    ...
    /**
     * Apply CDF-based histogram specification on a [0,1] channel.
     */
    fun applyChannelCdf(
        channel: FloatArray,
        ...
        // For each inputCdf value, find corresponding x in target
        ...
        // Apply transfer to channel
        val out = FloatArray(channel.size)
        for (i in channel.indices) {
            val lIn = channel[i].coerceIn(0f, 1f)
            val lMapped = interpUniform(lIn, transfer, numBins)
            out[i] = (black.toFloat() + lMapped * span).coerceIn(0f, 1f)
        }
    ...
    /**
     * Run the full image processing pipeline on a Bitmap.
     * Returns the processed Bitmap.
     */
    fun process(input: Bitmap, gamutLut: FloatArray? = null, skipDownscale: Boolean = false): ProcessingResult {
        val t0 = System.nanoTime()
        val timings = mutableMapOf<String, Long>()
        val steps = mutableListOf<StepTiming>()
        fun ms(since: Long): Long = (System.nanoTime() - since) / 1_000_000
        fun mark(name: String, since: Long, isIO: Boolean = false) {
            val m = ms(since); timings[name] = m; steps.add(StepTiming(name, m, isIO)); Log.i(TAG, "$name: ${m}ms")
        }

        // Downscale large images to avoid OOM (skip in benchmark mode)
        val scaled = if (skipDownscale) input else downscaleIfNeeded(input)
        val width = scaled.width
        val height = scaled.height
        val pixelCount = width * height
        Log.i(TAG, "Image size: ${width}x${height} ($pixelCount px)")

        // Step 0: Extract pixels from (possibly downscaled) bitmap  [I/O]
        var tStep = System.nanoTime()
        val pixels = IntArray(pixelCount)
        scaled.getPixels(pixels, 0, width, 0, 0, width, height)
        mark("Step 0 getPixels", tStep, isIO = true)

        // Step 1: sRGB EOTF (uint8 → linear float) via LUT
        tStep = System.nanoTime()
        val linearRgb = FloatArray(pixelCount * 3)
        val eotf = EOTF_LUT  // local ref avoids repeated lazy-delegate access
        for (i in 0 until pixelCount) {
            val color = pixels[i]
            val idx = i * 3
            linearRgb[idx]     = eotf[(color shr 16) and 0xFF]
            linearRgb[idx + 1] = eotf[(color shr  8) and 0xFF]
            linearRgb[idx + 2] = eotf[ color         and 0xFF]
        }
        mark("Step 1 sRGB EOTF", tStep)

        // Step 2: Linear RGB → OKLab
        tStep = System.nanoTime()
        val (L, a, bOk) = OkLab.linearRgbToOklab(linearRgb, pixelCount)
        mark("Step 2 RGB→OKLab", tStep)

        // Step 3: Build gamut LUT (or reuse provided)
        tStep = System.nanoTime()
        val lut = gamutLut ?: OkLab.buildGamutLut()
        mark("Step 3 Gamut LUT", tStep)

        // Step 4: Compute relative chroma
        tStep = System.nanoTime()
        val (cRel, hue) = OkLab.relativeChroma(L, a, bOk, lut, pixelCount)
        mark("Step 4 Relative chroma", tStep)

        // Step 5: Histogram computation + capping
        tStep = System.nanoTime()
        val rawHistL = computeHistogram(L.also { arr ->
            for (i in arr.indices) arr[i] = arr[i].coerceIn(0f, 1f)
        })
        val capL = rawHistL.maxOrNull() ?: 0.0
        val cappedHistL = capHistogram(rawHistL, capL)
        ...
        val stepsL = fittedL["steps"]?.toInt() ?: 0
        val stepsC = fittedC["steps"]?.toInt() ?: 0
        mark("Step 5b Fit params ($stepsL+$stepsC steps analytical)", tStep)

        // Step 6: CDF matching on L channel
        tStep = System.nanoTime()
        val tL = fittedL["t"]!!
        val sL = fittedL["s"]!!
        val cL = fittedL["c"]!!
        ...
        val lOut = applyChannelCdf(L, cappedHistL, tL, sL, cL, gL, blackL, whiteL)
        mark("Step 6 CDF match L", tStep)

        // Step 7: CDF matching on relative chroma
        tStep = System.nanoTime()
        val tC = fittedC["t"]!!
        val sC = fittedC["s"]!!
        val cC = fittedC["c"]!!
        ...
        val cRelOut = applyChannelCdf(cRel, cappedHistC, tC, sC, cC, gC, blackC, whiteC)
        mark("Step 7 CDF match C", tStep)

        // Step 8: Reconstruct (a, b)
        tStep = System.nanoTime()
        val (a2, b2) = OkLab.reconstructAb(cRelOut, hue, lOut, lut, pixelCount)
        mark("Step 8 Reconstruct ab", tStep)

        // Step 9: Zone weights
        tStep = System.nanoTime()
        val cdfBins = FloatArray(NUM_BINS) { it.toFloat() / (NUM_BINS - 1).toFloat() }
        val outHistL = computeHistogram(lOut.also { arr ->
            for (i in arr.indices) arr[i] = arr[i].coerceIn(0f, 1f)
        })
        val outCdf = DoubleArray(NUM_BINS)
        var cdfCumSum = 0.0
        ...
        val outCdfFloat = FloatArray(NUM_BINS) { outCdf[it].toFloat() }

        val (wS, wM, wH) = OkLab.zoneWeightsCdf(lOut, cdfBins, outCdfFloat, pixelCount)
        mark("Step 9 Zone weights", tStep)

        // Step 10: Chroma offset
        tStep = System.nanoTime()
        var a3 = a2
        var b3 = b2
        val (a3r, b3r) = OkLab.chromaOffsetAb(
            a3, b3, wS, wM, wH,
            0f, 0f, 0f, 0f, 0f, 0f, pixelCount
        )
        a3 = a3r
        b3 = b3r
        mark("Step 10 Chroma offset", tStep)

        // Step 11: Gamut clamp (uses precomputed hue)
        tStep = System.nanoTime()
        val (aFinal, bFinal) = OkLab.gamutClampAb(lOut, a3, b3, lut, pixelCount, hue)
        mark("Step 11 Gamut clamp", tStep)

        // Step 12: OKLab → linear → sRGB OETF → uint8 via LUT
        tStep = System.nanoTime()
        val outLinear = OkLab.oklabToLinearRgb(lOut, aFinal, bFinal, pixelCount)

        val outPixels = IntArray(pixelCount)
        val oetf = OETF_LUT  // local ref avoids repeated lazy-delegate access
        for (i in 0 until pixelCount) {
            val idx = i * 3
            val rEnc = oetf[(outLinear[idx].coerceIn(0f, 1f) * 4095f).toInt()]
            val gEnc = oetf[(outLinear[idx + 1].coerceIn(0f, 1f) * 4095f).toInt()]
            val bEnc = oetf[(outLinear[idx + 2].coerceIn(0f, 1f) * 4095f).toInt()]
            outPixels[i] = (0xFF shl 24) or (rEnc shl 16) or (gEnc shl 8) or bEnc
        }
        mark("Step 12 OKLab→sRGB", tStep)
        ...
        timings["Total"] = ms(t0); Log.i(TAG, "Total pipeline: ${timings["Total"]}ms")
